...
TGL_EXPORT void glNewList(unsigned int list,int mode)
{
  GLList *l;
  GLContext *c=gl_get_context();
...
TGL_EXPORT void glEndList(void)
{
  GLContext *c=gl_get_context();
  GLParam p[1];
...
TGL_EXPORT int glIsList(unsigned int list)
{
  GLContext *c=gl_get_context();
  GLList *l;
  l=find_list(c,list);
...
TGL_EXPORT unsigned int glGenLists(int range)
{
  GLContext *c=gl_get_context();
  int count,i,list;
  GLList **lists;
